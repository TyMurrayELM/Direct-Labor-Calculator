...
export default function SprayForecastPage() {
  ...
  const validBranchId = selectedBranchId && selectedBranchId !== 'phoenix' && selectedBranchId !== 'encore' ? selectedBranchId : null;
  ...
  // Check if Phoenix (combined) or Encore (all branches) view is selected
  ...
  const selectedBranch = isEncoreView
    ? { name: 'Encore (All Branches)', color: '#2563EB' }
    : isPhoenixView
    ? { name: 'Phoenix (Combined)', color: '#EA580C' }
  ...
  // Calculate Spray metrics for a given maintenance and onsite revenue
  const calculateSprayMetrics = (maintenanceRevenue, onsiteRevenue) => {
    const combinedRevenue = maintenanceRevenue + onsiteRevenue;
    const sprayRevenue = combinedRevenue * SPRAY_REVENUE_PERCENT;
    const laborRevenue = sprayRevenue * LABOR_REVENUE_PERCENT;
    const nonLaborRevenue = sprayRevenue - laborRevenue;
  ...
  const monthlyData = months.map(month => {
    ...
    return {
      month,
      ...calculateSprayMetrics(maintenanceRevenue, onsiteRevenue)
    };
  });
  ...
  const totals = monthlyData.reduce((acc, d) => ({
    ...
    sprayRevenue: acc.sprayRevenue + d.sprayRevenue,
    ...
    try {
      ...
      const res = await fetch('/api/slack/spray-forecast', {
        ...
        body: JSON.stringify({
          branchName,
          year: selectedYear,
          sprayRevenue: currentMonthData.sprayRevenue,
  ...
  const exportToCSV = () => {
    try {
      ...
      const branchName = isEncoreView ? 'Encore_All' : isPhoenixView ? 'Phoenix_Combined' : (selectedBranch.name || 'Unknown').replace(/\s+/g, '_');
      ...
      // Labor Cost
      rows.push(['Labor Cost (@$40/hr)', ...monthlyData.map(d => d.laborCost), totals.laborCost]);

      // FTEs Required
      rows.push(['FTEs Required', ...monthlyData.map(d => d.ftes.toFixed(1)), avgFtes.toFixed(1) + ' (avg)']);

      // Crews Required
      rows.push(['Crews (1-person)', ...monthlyData.map(d => Math.ceil(d.crews)), Math.ceil(avgCrews) + ' (avg)']);
      ...
      link.setAttribute('download', `Spray_FTE_Forecast_${branchName}_${selectedYear}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="flex items-center justify-center min-h-screen bg-red-50">
        <div className="w-full max-w-2xl px-6">
          <div className="mb-6 flex items-center gap-3">
            <div className="h-7 w-7 rounded-full border-[3px] border-red-600 border-t-transparent animate-spin" />
            <p className="text-lg font-semibold text-black">Loading forecast data...</p>
          </div>
          <div className="space-y-3">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="flex gap-4 animate-pulse">
                <div className="h-4 bg-red-200 rounded w-44" />
                <div className="h-4 bg-red-100 rounded w-24" />
                <div className="h-4 bg-red-200 rounded w-20" />
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 bg-red-50 min-h-screen">
      <div className="bg-white shadow-xl rounded-xl overflow-hidden border border-gray-100">
        {/* Header */}
        <div className="bg-gradient-to-r from-white to-gray-100 p-4 border-b border-gray-200"
          style={{ borderTop: `4px solid ${selectedBranch.color || '#DC2626'}` }}>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
              {/* Spray Icon */}
              <div className="bg-red-100 p-2 rounded-lg">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                </svg>
              </div>
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Spray FTE Forecast</h1>
                <p className="text-sm text-gray-700 mt-1">Based on Maintenance + Onsite Revenue projections</p>
              </div>
            </div>
          ...
          {/* Constants Display */}
          <div className="flex flex-wrap gap-4 text-sm bg-red-50 rounded-lg p-4 border border-red-200">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-700">Spray Target:</span>
              <span className="text-red-700 font-semibold">12% of (Maint + Onsite)</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-red-700 font-semibold">${BILLING_RATE}/hr</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-700">Labor Cost:</span>
              <span className="text-red-700 font-semibold">${HOURLY_COST}/hr</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-700">Crew Size:</span>
              <span className="text-red-700 font-semibold">{CREW_SIZE} person</span>
            </div>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-red-800 text-white text-sm">
                <th className="px-3 py-2 text-left font-semibold sticky left-0 bg-red-800 z-10">Metric</th>
                {months.map(month => (
                  <th key={month} className="px-2 py-2 text-center font-semibold min-w-20">
                    {month}
                  </th>
                ))}
                <th className="px-3 py-2 text-center font-semibold bg-red-900 min-w-24">Total</th>
              </tr>
            </thead>
            <tbody className="text-sm">
              ...
              {/* Spray Revenue Target Row */}
              <tr className="bg-red-50 border-b border-red-200">
                <td className="px-2 py-2 font-medium text-gray-700 sticky left-0 bg-red-50 z-10">
                  Spray Revenue (12% of Maint + Onsite)
                </td>
                {monthlyData.map(d => (
                  <td key={d.month} className="px-2 py-2 text-center text-red-700 font-medium">
                    {d.sprayRevenue > 0 ? formatCurrency(d.sprayRevenue) : '—'}
                  </td>
                ))}
                <td className="px-2 py-2 text-center font-bold text-red-700 bg-red-100">
                  {formatCurrency(totals.sprayRevenue)}
                </td>
              </tr>
              ...
              <tr className="bg-rose-50 border-b border-rose-200">
                ...
                <td className="px-2 py-2 text-center font-semibold text-rose-700 bg-rose-100">
              ...
              <tr className="bg-rose-50/50 border-b border-rose-100">
                ...
                <td className="px-2 py-1.5 text-center text-xs text-gray-700 bg-rose-100/50">
                  {formatCurrency(totals.nonLaborRevenue)}
                </td>
              </tr>

              {/* Billable Hours Row */}
              <tr className="bg-red-50/70 border-b border-red-200">
                <td className="px-2 py-2 font-medium text-gray-700 sticky left-0 bg-red-50/70 z-10">
                  Billable Hours (@${BILLING_RATE}/hr)
                </td>
                {monthlyData.map(d => (
                  <td key={d.month} className="px-2 py-2 text-center text-red-600">
                    {d.billableHours > 0 ? formatNumber(d.billableHours, 0) : '—'}
                  </td>
                ))}
                <td className="px-2 py-2 text-center font-semibold text-red-600 bg-red-100/70">
        ...
        {/* Summary Cards */}
        {totals.sprayRevenue > 0 && (
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-5 text-white shadow-lg">
                <div className="text-red-100 text-sm font-medium mb-1">Annual Spray Revenue Target</div>
                <div className="text-2xl font-bold">{formatCurrency(totals.sprayRevenue)}</div>
                <div className="text-red-200 text-xs mt-1">12% of (Maint + Onsite)</div>
              </div>
