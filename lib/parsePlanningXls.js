...
/**
 * Normalize account code by stripping leading zeros from each sub-segment.
 * Matches lib/parsePnlXls.js exactly — trailing zeros are PRESERVED because
 * codes like "6152.10" (Specialist Wages) and "6152.1" (Administrative Wages)
 * are different real accounts and must not collapse.
 *
 * Examples: "5101.01" → "5101.1", "5050.2.03" → "5050.2.3", "6152.10" → "6152.10"
 */
...
export function parsePlanningSheet(arrayBuffer, branchFilter) {
  ...
  // Formatted-text view of the same rows. Account codes stored as Excel
  // NUMBERS lose trailing zeros through the raw view (6152.10 → 6152.1,
  // which is a different account); the formatted text preserves them.
  const rowsText = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: false });
  ...
  // branchFilter can be a string or an array of strings (match any)
  const normalizeBranch = (s) => s.toLowerCase().trim()
    .replace(/\bphx\b/g, 'phoenix')
    .replace(/\blv\b/g, 'las vegas');
  const branchFilters = branchFilter
    ? (Array.isArray(branchFilter) ? branchFilter : [branchFilter]).map(normalizeBranch)
    : null;

  console.log(`[parsePlanningSheet] type=${type}, headerRow=${headerRowIdx}, totalRows=${rows.length}, branchFilters=${JSON.stringify(branchFilters)}`);
  ...
  for (let i = dataStartIdx; i < rows.length; i++) {
    ...
    // For numeric cells, use the formatted text so trailing zeros survive
    // (the raw number 6152.10 is indistinguishable from 6152.1).
    const col1 = typeof col1raw === 'number'
      ? String(rowsText[i]?.[1] ?? col1raw).trim()
      : String(col1raw).trim();
    ...
    if (branchFilters) {
      ...
      if (!branchFilters.some(f => branchCol.includes(f))) continue;
    }
