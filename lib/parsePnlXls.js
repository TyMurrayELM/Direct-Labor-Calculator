...
export function parsePnlXls(arrayBuffer) {
  ...
  // Deduplicate detail rows that share the same normalized account_code
  // (NetSuite exports sometimes include both "5101.01" and "5101.1" as separate rows).
  // Sum their month values and keep the first occurrence.
  const ALL_MONTHS = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'];
  {
    const seen = new Map(); // account_code → index in lineItems
    const toRemove = new Set();
    for (let i = 0; i < lineItems.length; i++) {
      const li = lineItems[i];
      if (li.row_type !== 'detail' || !li.account_code) continue;
      if (seen.has(li.account_code)) {
        const firstIdx = seen.get(li.account_code);
        const first = lineItems[firstIdx];
        for (const mk of ALL_MONTHS) {
          first[mk] = (first[mk] || 0) + (li[mk] || 0);
        }
        toRemove.add(i);
      } else {
        seen.set(li.account_code, i);
      }
    }
    if (toRemove.size > 0) {
      const filtered = lineItems.filter((_, i) => !toRemove.has(i));
      lineItems.length = 0;
      lineItems.push(...filtered);
      for (let j = 0; j < lineItems.length; j++) lineItems[j].row_order = j;
    }
  }
...
/**
 * Normalize account code by stripping leading AND trailing zeros from each
 * sub-segment.  NetSuite / Excel exports are inconsistent: the same
 * sub-account may appear as "01", "1", or "10" (Excel decimal padding).
 *
 * Examples: "6152.10" → "6152.1", "5101.01" → "5101.1", "5050.2.03" → "5050.2.3"
 */
function normalizeAccountCode(code) {
  if (!code) return code;
  return code.split('.').map((seg, i) => {
    if (i === 0) return seg;
    let s = seg.replace(/^0+/, '') || '0';   // strip leading zeros
    s = s.replace(/0+$/, '') || '0';          // strip trailing zeros (Excel padding)
    return s;
  }).join('.');
}
